...
class UpcomingTVShowController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $type = Type::where('type', 5)->latest()->get();

            return view('admin.upcoming_tv_show.index', ['type' => $type]);
        } catch (Exception $e) {
            ...
            if ($request == true) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $btn .= '<a href="' . route("upcomingtvshowEdit", $row->id) . '" Title="Edit"><img src="' . url("assets/imgs/edit.png") . '" /></a> ';
                        $btn .= '<a href="' . route("upcomingtvshowDelete", $row->id) . '" title="Delete" onclick="return confirm(\'Are you sure !!! You want to Delete this TV Show ?\')"><img src="' . asset("assets/imgs/trash.png") . '" /></a></div>';
                        return $btn;
                    })
                    ->addColumn('details', function ($row) {
                        $btn = '<a href="' . route("upcomingTVShowDetail", $row->id) . '" class="btn text-white p-1 font-weight-bold" style="background:#17a2b8;">More Details</a> ';
                        return $btn;
                    })
                    ->addColumn('season', function ($row) {
                        $btn = '<a href="' . route("upcomingTVShowVideo", $row->id) . '" class="btn text-white p-1 font-weight-bold" style="background:#006a4e;"> Episode List</a> ';
                        return $btn;
                    })
                    ->rawColumns(['action', 'details', 'season'])
                    ->make(true);
            } else {
                return view('admin.upcoming_tv_show.index');
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    ...
    public function add()
    {
        try {
            ...
            return view('admin.upcoming_tv_show.add', ['channel' => $channel, 'category' => $category, 'language' => $language, 'cast' => $cast, 'type' => $type]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function edit($id)
    {
        try {
            ...
            $type = Type::where('type', 2)->Orwhere('type', 5)->orderBy('type', 'desc')->get();

            return view('admin.upcoming_tv_show.edit', ['result' => $TVShow, 'type' => $type, 'channel' => $channel, 'category' => $category, 'language' => $language, 'cast' => $cast]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
        ...
        try {
            ...
            if (isset($TVShow->id)) {
                ...
                if ($TVShow->save()) {
                    ...
                    // Banner Update && Show Episode Video Type Change
                    if ($TVShow->video_type != 5) {
                        Banner::where('video_type', 5)->where('video_id', $TVShow->id)->where('upcoming_type', 2)->update(['type_id' => $TVShow->type_id, 'video_type' => $TVShow->video_type, 'upcoming_type' => 0]);

                        TVShowVideo::where('show_id', $TVShow->id)->update(['video_type' => $TVShow->video_type]);
                    }
    ...
    public function delete($id)
    {
        try {
            $TVShow = TVShow::where('id', $id)->first();
            $App_Section = App_Section::whereRaw("find_in_set('" . $TVShow->id . "',app_section.video_id)")->where('video_type', 5)->first();
            ...
            if ($App_Section) {
                ...
                if ($TVShow->delete()) {
                    ...
                    return redirect()->route('upcomingTVShow')->with('success', __('Label.Data Delete Successfully'));
                }
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
    ...
    public function TVShowDetail($id)
    {
        try {
            ...
            return view('admin.upcoming_tv_show.detail_page', ['tvshow' => $TVShow, 'channel' => $channel, 'category' => $category, 'language' => $language, 'cast' => $cast]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
        ...
        try {
            ...
            if ($input_search != null && isset($input_search)) {
                ...
                if ($input_session != 0) {
                    ...
                    $video_list = TvShowVideo::where('name', 'LIKE', "%{$input_search}%")
                        ->where('show_id', $id)
                        ->where('session_id', $input_session)
                        ->with('session')
                ...
                if ($input_session != 0) {
                    $video_list = TvShowVideo::where('show_id', $id)->where('session_id', $input_session)->with('session')->orderBy('id', 'desc')->paginate(15);
                } else {
                    $video_list = TvShowVideo::where('show_id', $id)->with('session')->orderBy('id', 'desc')->paginate(15);
                }
            }
            ...
            return view('admin.upcoming_tv_show.video_list', ['tvshowId' => $id, 'result' => $video_list, 'session' => $session]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function TVShowvideoadd($id)
    {
        try {
            ...
            return view('admin.upcoming_tv_show.add_video', ['tvshowId' => $id, 'session' => $session]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function TVShowvideoedit($show_id, $id)
    {
        try {
            ...
            $session = Session::get();
            if ($result) {
                return view('admin.upcoming_tv_show.edit_video', ['tvshowId' => $show_id, 'session' => $session, 'result' => $result]);
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    ...
    public function TVShowvideoupdate(Request $request)
    {
        try {
            ...
            if (isset($TVShowVideo->id)) {
                ...
                $TVShowVideo->popup_id = $request->popup_id;
