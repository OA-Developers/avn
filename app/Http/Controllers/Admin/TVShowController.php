...
class TVShowController extends Controller
{
    ...
    public function TVShowvideoadd($id)
    {
        try {
            ...
            $popups = Popup::all();
            return view('admin.upcoming_tv_show.add_video', ['tvshowId' => $id, 'session' => $session, 'popups' => $popups]);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function TVShowvideoedit($show_id, $id)
    {
        try {
            ...
            $popups = Popup::all();
            if ($result) {
                return view('admin.tv_show.edit_video', ['tvshowId' => $show_id, 'session' => $session, 'result' => $result, 'popups' => $popups]);
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
